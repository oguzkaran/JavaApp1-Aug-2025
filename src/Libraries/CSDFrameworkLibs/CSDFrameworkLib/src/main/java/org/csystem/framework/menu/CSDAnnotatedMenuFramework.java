...
public class CSDAnnotatedMenuFramework {
    ...
    {
        var optionMethods = cls.getDeclaredAnnotationsByType(OptionMethods.class)[0];
        var nameMethod = cls.getDeclaredMethod(optionMethods.name());
        var optionStringMethod = cls.getDeclaredMethod(optionMethods.optionString());
        var doOptionMethod = cls.getDeclaredMethod(optionMethods.doOption());
    ...
    {
        try {
            createMenuInfoList(ReflectionUtil.getAnnotatedClassesByJars(m_dirPath, OptionMethods.class));
            if (m_sorted)
                m_menuInfoList.sort(CSDAnnotatedMenuFramework::compareCallback);
        }
        catch (Exception e) {
